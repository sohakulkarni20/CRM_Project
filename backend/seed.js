...
/* ─────────────────────────────────────────────────────────────
   DEMO USER / PROJECT OWNER
   ───────────────────────────────────────────────────────────── */

const DEMO_USER = {
  name: "Soha Kulkarni",
  email: "soha@crmproject.com",
  password: "Test@1234",
  company: "Soha CRM",
};
...
const COMPANIES = [
  ...
  ["Kruger Industrial Smoothing", "kruger.com"],
  ...
  ["Soylent Green", "lsgreen.io"],
  ["Cogswell Cogs", "cogswell.com"],
  ["Stark Labs", "starklabs.com"],
...
const run = async () => {
  ...
  for (let i = 0; i < 40; i++) {
    const [company, domain] =
      usedCompanies[i] || pick(COMPANIES);
    ...
    leadDocs.push({
      ...
      tags: pickSome(
        ["saas", "enterprise", "smb", "priority"],
        rand(0, 2)
      ),
      order: stageOrder[status]++,
      createdAt: daysAgo(ageDays),
      updatedAt: daysAgo(
        rand(0, Math.min(ageDays, 14))
      ),
    });
  }
  ...
  console.log(
    `\n✅ Seed complete for ${DEMO_USER.name}'s workspace!`
  );
